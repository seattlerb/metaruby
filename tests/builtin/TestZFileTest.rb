...
class TestZFileTest < FileInfoTest
	...
	tests << [ :executable?, ?x,    "/dev/tty",          false ]
      ...
      for meth, t, file, result in tests
        if file
          ...
          if meth
            assert_equal(result, ZFileTest.send(meth, file), 
                         "ZFileTest.#{meth}(#{file})")
          end
        end
      end
      ...
      Windows.known_problem do
          ...
          assert_equal(Process.euid == 0, ZFileTest.send(:writable?, filew),
                       "ZFileTest.writable?(#{filew})")
      end
