...
class TestZDir < Rubicon::TestCase
  ...
  def test_s_chdir
    start = ZDir.getwd
    assert_exception(Errno::ENOENT)       { ZDir.chdir "_wombat" }
    assert_equal(0,                         ZDir.chdir("_test"))
    assert_equal(File.join(start, "_test"), ZDir.getwd)
  ...
  def test_s_entries
    assert_exception(Errno::ENOENT)      { ZDir.entries "_wombat" } 
    assert_exception(Errno::ENOENT)      { ZDir.entries "_test/file*" } 
  ...
  def test_s_foreach
    got = []
    entry = nil
    assert_exception(Errno::ENOENT) { ZDir.foreach("_wombat") {}}
  ...
  def test_s_new
    assert_exception(ArgumentError) { ZDir.new }
    assert_exception(ArgumentError) { ZDir.new("a", "b") }
    assert_exception(Errno::ENOENT) { ZDir.new("_wombat") }

    assert_equal(ZDir, Dir.new(".").class)
  end

  def test_s_open
    assert_exception(ArgumentError) { ZDir.open }
    assert_exception(ArgumentError) { ZDir.open("a", "b") }
    assert_exception(Errno::ENOENT) { ZDir.open("_wombat") }
  ...
  def test_s_rmdir
    assert_kindof_exception(SystemCallError)    { ZDir.rmdir "_wombat" } 
    assert_kindof_exception(SystemCallError)    { ZDir.rmdir "_test" } 
    delete_test_dir
  ...
  def test_s_unlink
    assert_kindof_exception(SystemCallError)    { ZDir.unlink "_wombat" } 
    assert_kindof_exception(SystemCallError)    { ZDir.unlink "_test" } 
    delete_test_dir
